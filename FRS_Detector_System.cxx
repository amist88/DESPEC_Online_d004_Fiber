...
void FRS_Detector_System::VFTX_Calibration(int module, int channel) {
  ...
  std::ofstream f_int;
  std::ofstream f_cal;
  f_cal.open(Form("Configuration_Files/VFTX_Calib/VFTX_%05d_Bin2Ps_ch%02d.dat",l_VFTX_SN[module],channel),std::ios::out);
  f_int.open(Form("Configuration_Files/VFTX_Int/VFTX_%05d_INT_ch%02d.txt",l_VFTX_SN[module],channel),std::ios::out);
...
void FRS_Detector_System::m_VFTX_Bin2Ps(){
  ...
  for(int mod=0; mod<VFTX_N; mod++){
    for(int ch=0; ch<VFTX_MAX_CHN; ch++){
      std::ifstream in;
      in.open(Form("Configuration_Files/VFTX_Calib/VFTX_%05d_Bin2Ps_ch%02d.dat",l_VFTX_SN[mod],ch));
      if(!in.is_open()){
