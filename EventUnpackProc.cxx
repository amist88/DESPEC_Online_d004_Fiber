...
EventUnpackProc::EventUnpackProc(const char* name) : TGo4EventProcessor(name)
{
 ...
 // Detector_Systems[2] = !Used_Systems[2] ? nullptr : new PLASTIC_VME_Detector_System();
  ...
  Detector_Systems[3] =  new FATIMA_Detector_System();

  Detector_Systems[4] = new FATIMA_TAMEX_Detector_System();
...
//         fOutput->fFRS_ts2 = RAW->get_FRS_ts2();
     ...
     if (Used_Systems[4]&& PrcID_Conv==4){
         ...
         ///------------------------------------------------------------///
                                                /**Output FATIMA TAMEX **/
                                                ///FATIMA AND PLASTIC TAMEX NEEDS TESTING!!! 26.3.20
        ///--------------------------------------------------------------------------------------------///
  ...
  for (int i=0; i<RAW->get_FATIMA_tamex_hits(); i++){///Loop over tamex ID's
		...
		//cout<<"chan_fat "<<chan_fat<<endl;
           ...
           if(chan_fat>-1) {
            ...
            fOutput->fFat_Lead_PMT[chan_fat][N1] = RAW->get_FATIMA_lead_T(i,j);
              //   cout<<" fOutput->fFat_Lead_PMT[chan_fat][N1] " << fOutput->fFat_Lead_PMT[chan_fat][N1] << " chan_fat " << chan_fat << " N1 " << N1 << endl;
           }
	...
	// cout<<"222Phys_Channel_Trail_Fat[i][j] " << Phys_Channel_Trail_Fat[i][j] <<" RAW->get_PLASTIC_physical_channel(i, j) " <<RAW->get_PLASTIC_physical_channel(i, j) << " i " << i << " j " << j << endl;
                
                
		//cout << " RAW->get_PLASTIC_physical_channel(i, j) "<<RAW->get_PLASTIC_physical_channel(i, j)<<" Phys_Channel_Trail_Fat[i][j] " <<Phys_Channel_Trail_Fat[i][j]  <<" TAMEX_Fat_ID " <<TAMEX_Fat_ID[i][RAW->get_PLASTIC_physical_channel(i, j)] << " i " << i << " j " << j << endl;
		...
		// cout<<"chan_fat " << chan_fat <<endl;
              ...
              //cout<<"fOutput->fFat_Trail_PMT[chan_fat][N1] " << fOutput->fFat_Trail_PMT[chan_fat][N1] << " chan_fat " << chan_fat << " N1 "<< N1 <<endl;
                                }
                            }       
                    }
            }
        ...
        ///--------------------------------------------------------------------------------------------///
            int bPlasfired[3];
            int Phys_Channel_Lead_bPlas[3][256];
            int Phys_Channel_Trail_bPlas[3][256];
            int N1 =0;
       ...
       for (int i=0; i<RAW->get_PLASTIC_tamex_hits(); i++){///Loop over tamex ID's
           ...
           //fOutput->fbPlas_TAMEX_ID = i;
            ...
            // if(i==2)   cout<<"chan " << chan <<" i " << i << endl;
  ...
  //cout<<"event " << event_number <<" Lead " << fOutput->fbPlas_Lead_PMT[bPlasdetnum][chan][N1]<<" N1 " << N1 << " bPlasdetnum " << bPlasdetnum << " chan " << chan << " j " << j << endl;
               ...
               if(RAW->get_PLASTIC_CH_ID(i,j) % 2 == 0){ //Trail even j
                ...
                if(N1>-1){
         //  cout<<"chan " << chan << " bPlasdetnum " << bPlasdetnum << " N1 " << N1 <<endl;
     ...
     /**------------------GALILEO Raw Energy -----------------------------------------**/
      GALILEO_hits = RAW->get_GALILEO_am_Fired();
	   ...
	   if(RAW->get_GALILEO_Det_id(i)>-1){
           //cout<<"T " << RAW->get_GALILEO_Chan_T(i)*1E-9 <<endl;
           hFebTime->Fill(RAW->get_GALILEO_Chan_T(i)*10E-9);
	 //  cout<<"GalID " << GalID<< " RAW->get_GALILEO_Det_id(i) " <<RAW->get_GALILEO_Det_id(i) << " RAW->get_GALILEO_Crystal_id(i) " <<RAW->get_GALILEO_Crystal_id(i)<< endl;
         GalID = RAW->get_GALILEO_Det_id(i) * 3 + RAW->get_GALILEO_Crystal_id(i);
        tmpGAL[GalID] = RAW->get_GALILEO_Chan_E(i);
       // cout<<"GalID " <<GalID << " RAW->get_GALILEO_Chan_E(i) " <<RAW->get_GALILEO_Chan_E(i) << " i "<< i << endl;
