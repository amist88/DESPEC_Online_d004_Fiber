...
: TGo4Parameter(name)
{
  ...
  if (file.fail()) {
        cout << "ERROR: CorrelParameter - Could not open file: Configuration_Files/Correlations.dat ! Params set to nominal\n";    
         ...
         GbPlas_GAL_TLow = -1000;
         GbPlas_GAL_THigh = 1000;
...
cout << "CorrelParameter - reading from Configuration_Files/Correlations.dat";
       ...
       ///bPlas Fatima WR Time Gate
       if(IsData(file)) file >> GbPlas_FAT_TLow >>GbPlas_FAT_THigh;
       ///FRS Galileo WR Time Gate
       if(IsData(file)) file >> GFRS_GAL_TLow >>GFRS_GAL_THigh;
       ///bPlas Galileo WR Time Gate
       if(IsData(file)) file >> GbPlas_GAL_TLow >>GbPlas_GAL_THigh;
...
Int_t CorrelParameter::PrintParameter(Text_t *buf, Int_t)
{
  ...
  cout << "//////bPlas Fatima WR Time Gate\n";
  cout << "////Lower: " << GbPlas_FAT_TLow << "  \t Upper: = " <<  GbPlas_FAT_THigh <<  endl;
  
  cout << "//////FRS Galileo WR Time Gate\n";
  cout << "////Lower: " << GFRS_GAL_TLow << "  \t Upper: = " <<  GFRS_GAL_THigh <<  endl;
  
  cout << "//////bPlas Galileo WR Time Gate\n";
  cout << "////Lower: " << GbPlas_GAL_TLow << "  \t Upper: = " <<  GbPlas_GAL_THigh <<  endl;
...
Bool_t CorrelParameter::UpdateFrom(TGo4Parameter *pp)
{
  if(pp->InheritsFrom("CorrelParameter"))
  {
    CorrelParameter *from = (CorrelParameter *) pp;
      ...
      GFRS_GAL_TLow = from->GFRS_GAL_TLow; 
      GFRS_GAL_THigh = from->GFRS_GAL_THigh; 
      GbPlas_GAL_TLow = from->GbPlas_GAL_TLow; 
      GbPlas_GAL_THigh = from->GbPlas_GAL_THigh; 
